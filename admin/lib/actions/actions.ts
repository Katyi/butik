...
export const getImageUploadOld = async (file: any) => {
...
export const getImageUpload = async (file: any, setFile: any) => {
  const formData = new FormData();
...
export const imageDelete = async (id: any) => {
  try {
    await fetch(`api/uploadImage/deleteImage/${id}`);
  } catch (err) {
    console.log(err);
  }
};
